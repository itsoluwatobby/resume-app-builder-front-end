...
export const HomeCards = ({ reverse, top, bottom, upper }) => {
  ...
  return (
      <article className={`flex transform-gpu w-fit transition-transform skew-x-2 ${bottom ? 'md:translate-x-28' : ''} ${upper ? 'md:translate-y-10' : ''} ${reverse ? 'flex-col-reverse maxscreen:hidden absolute bottom-5 right-14' : 'flex-col'} ${top ? 'right-60 bottom-10' : ''} border-b shadow-lg rounded-md whitespace-pre-wrap md:w-[30%] bg-opacity-60 bg-slate-900 ${theme === 'light' ? 'text-black' : 'text-white'} p-2`}>
