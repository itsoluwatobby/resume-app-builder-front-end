...
const Register = () => {
  ...
  return (
    <main className='absolute top-28 left-[35%] midscreen:left-[15%] max-h-64 midscreen:w-[75%] w-[35%]'>
      <div 
        onClick={() => setError('')}
        className='p-2 flex items-center flex-col gap-2 h-full w-full border bg-gray-300 rounded-md shadow-lg'>
        <h1 className='flex-none uppercase font-[600] border-b-2 border-gray-400 text-[20px]'>User Registration</h1>
        <form onSubmit={handleRegister} className='font-medium flex-auto flex flex-col w-full p-1'>
          ...
          onClick={() => setError(false)}
            type='submit'
            className='w-fill p-2 bg-green-500 rounded-md mt-2 cursor-pointer hover:bg-green-400 active:bg-green-500 transition duration-150 ease-in-out text-[18px]'  
          >Sign Up</button>
        </form>
        <p
          onClick={() => {
